...
- **Design source of truth & governance:** all visual changes originate in `src/shared/ui/theme/` and are documented in [`BRAND.md`](BRAND.md) (palette, typography, spacing, radius/shadow, **motion**, `level`, `zIndex`, `layout`, interactive states, motion rules incl. `prefers-reduced-motion`). A new color/size/duration/easing/width enters **`tokens.ts` first**; components consume it via `theme.*` — no stray hex/`px`/`ms`/bézier in components (`px` only for hairline borders). If a component "needs" a value not in the theme, add it to the theme rather than hardcoding.
